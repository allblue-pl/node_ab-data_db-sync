...
// export default async function createAndroidClass_Async(packageName: string, 
//         packagePath: string, tableDef: TableDef): Promise<void> {
//     createClass(packageName, packagePath, tableDef);
// };


// let createClass = (packageName: string, packagePath: string, table: TableDef) => {
//     let content = '';
//     content += 
// `package ${packageName};
...
//     fs.writeFileSync(path.join(packagePath, `T${table.name}.java`), 
//            content.replaceAll("\n", "\r\n"));
//     abLog.success(`Saved: ${table.name}.`);
// };

// function getFieldDeclaration(fieldInfo): string {
//     return `new ABTableColumn("` + fieldInfo.name + 
//             `", SelectColumnType.` + 
//             getFieldType(fieldInfo.field) + `)`;
// }

// function getFieldGetColumn(table: TableDef, fieldInfo, index: number): string {
//     let content = null;
