...
import { findPackage, getETable, getETableVariant, type ETable, type ETableVariant } from "./helpers.ts";
import abDataDefToPHPStan from "./abDataDefToPHPStan.ts";
import { getFieldType, getPHPStanType } from "./createEspadaTableClass.ts";

function createEspadaTableVariantClass(scheme: DataScheme, packagePaths: Array<string>,
...
function createClass(scheme: DataScheme, packagePath: string, eTableVariant: ETableVariant,
        eTable: ETable): void {
    let content = '';
    content += 
 ...
 * @phpstan-type _T_TVR${eTableVariant.fullName} array{`;
    ...
    for (let [ columnName, field ] of eTableVariant.tableVariant.columns) {
        let phpStanType = getPHPStanType(scheme, field, "select");
